...
import { IUser, IUserId, User, UserId } from "./user/contracts";
    ...
    async getAdapter(appOptions) {
      ...
      return {
        ...
        async getUserByProviderAccountId(providerId, providerAccountId) {
          ...
          const account = await app.store.account.findOne({
            where: {
              accountId: AccountId({
                providerId,
                providerAccountId,
        ...
        async linkAccount(
          ...
          await app.store.account.add(account);
        ...
        async createSession(user) {
          ...
          await app.store.session.add(session);
        ...
        async getSession(sessionToken) {
          ...
          if (session && isSessionExpired(session)) {
            await app.store.session.remove({
              where: {
        ...
        async updateSession(session, force) {
          ...
          await app.store.session.add(refreshed);
        ...
        async createVerificationRequest(
          ...
          await app.store.verifcationRequest.add(verificationRequest);
        ...
        async getVerificationRequest(identifier, token, secret, provider) {
          ...
          const verifcationRequest = await app.store.verifcationRequest.findOne(
            {
              where: {
                hashedToken,
              },
          ...
          if (
            ...
            await app.store.verifcationRequest.remove({
              where: {
                hashedToken,
              },
            });
        ...
        async deleteVerificationRequest(identifier, token, secret, provider) {
          ...
          await app.store.verifcationRequest.remove({ where: { hashedToken } });
